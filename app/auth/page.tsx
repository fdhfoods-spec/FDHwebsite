...
function AuthContent() {
  ...
  const handleSendOtpSubmit = async (e: React.FormEvent) => {
    ...
    if (!isSupabaseConfigured() || !supabase) {
      console.warn('[Supabase Auth Diagnosis Warning] Supabase client configuration missing or placeholder. Proceeding via direct Twilio backend route.')
    }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col justify-between font-sans selection:bg-secondary selection:text-white">
      <Header />

      {/* Auth Main Card */}
      <main className="flex-1 flex items-center justify-center py-16 px-4">
        <div className="w-full max-w-md">
          <div className="bg-white border border-gray-100 rounded-3xl p-8 shadow-xl relative overflow-hidden space-y-6">
            ...
            <div className="relative z-10 space-y-6">
              ...
              {/* Status Alerts */}
              {errorMsg && (
                <div className="p-4 rounded-2xl bg-red-50 border border-red-100 text-red-600 text-xs font-medium space-y-2 animate-shake">
                  <div className="flex items-start gap-2">
                    <ShieldAlert className="w-4 h-4 text-red-500 shrink-0 mt-0.5" />
                    <span className="font-bold">{errorMsg}</span>
                  </div>
                  {diagnosticLog && (
                    <div className="mt-2 p-3 bg-gray-50 rounded-xl border border-red-200 text-[10px] font-mono text-foreground/80 leading-relaxed whitespace-pre-wrap">
              ...
              {step === 'phone' && (
                <form onSubmit={handleSendOtpSubmit} className="space-y-4">
                  ...
                  <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-200">
                    <button
                      ...
                      className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${
                        channel === 'sms' ? 'bg-secondary text-white shadow-sm' : 'text-foreground/60 hover:text-foreground hover:bg-white'
                      }`}
                    >
                      📱 SMS OTP
                    </button>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-extrabold uppercase tracking-wider text-foreground/50 block">Full Name (Optional)</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-gray-50 border border-gray-200 text-foreground text-xs py-3 pl-10 pr-4 rounded-xl outline-none focus:border-secondary transition-all font-medium"
                      />
                    </div>
                  </div>
                  ...
                  <Button
                    type="submit"
                    disabled={isLoading}
                    className="w-full py-3.5 bg-secondary hover:bg-secondary/90 text-white font-black rounded-xl text-xs uppercase tracking-wider transition-all shadow-md shadow-secondary/20 flex items-center justify-center gap-2 mt-2"
                  >
